...
def triton_matmul_kernel(a_ptr, b_ptr, c_ptr, bias_ptr,  #
    ...
    k_loop_limit = tl.cdiv(K, BLOCK_SIZE_K)
    if not EVEN_K:
        k_loop_limit -= 1

    for _ in range(0, k_loop_limit):
    ...
    if not EVEN_K:
        ...
        offs_k = k_loop_limit * BLOCK_SIZE_K + tl.arange(0, BLOCK_SIZE_K)
        a_ptrs = a_ptr + offs_am[:, None] * stride_am + offs_k[None, :] * stride_ak
        b_ptrs = b_ptr + offs_k[:, None] * stride_bk + offs_bn[None, :] * stride_bn
...
def run_triton_matmul(M: int, N: int, K: int, use_bias: bool, use_dot: bool) -> None:
    a: Tensor
    b: Tensor
    bias: Optional[Tensor]
    a, b, bias = gen_input(M, N, K, use_bias)
    triton_provider: str = "triton-dot" if use_dot else "triton-multreduce"
    matmul(triton_provider, a, b, bias)
...
def test_matmul(M: int, N: int, K: int, use_bias: bool) -> None:
    ...
    assert allclose(c_triton_dot, c_triton_multreduce), "Triton Dot and Triton Multreduce results don't match."
...
def run_benchmark(use_bias: bool) -> None:
    ...
    @triton.testing.perf_report(triton.testing.Benchmark(
        ...
        line_arg="provider",
        line_vals=["torch", "triton-dot", "triton-multreduce"],
        line_names=[f"PyTorch ({perf_unit})", f"Triton Dot ({perf_unit})", f"Triton Multreduce ({perf_unit})"],
        ylabel=f"{perf_unit}",
        args={},
    ...
    def benchmark(M: int, N: int, K: int, provider: str) -> tuple[float, float, float]:
        ...
        print(
            f"(M, N, K) = {(M, N, K)}, provider = {provider}, p20 = {p20_gibps} GiB/s, p50 = {p50_gibps} GiB/s, p80 = {p80_gibps} GiB/s"
        )
